...
本篇文章从自身实践出发，结合相关代码，总结`WebRTC`实现的基本流程。

<!--more-->

## 1. 引言

首先我们先看《`WebRTC`权威指南》上给出的流程图，从这张图，我们要明确两件事：
...
然后我们再介绍一下`WebRTC`中的专有名词，方便读者对下文的理解。
...
以下代码不能直接运行，因为我这里并没有实现信令服务器，如何实现信令服务器可自由选择（比如，socket.io、websocket等）。
...
MDN文档：[>>>点我进入](https://developer.mozilla.org/zh-CN/docs/Web/API/WebRTC_API)

WebRTC学习资料大全：[>>>点我进入](https://blog.csdn.net/foruok/article/details/53005728)
